...
class ReactNativeZoomableView extends Component {
  ...
  panAnim = new Animated.ValueXY({
    x: 0,
    y: 0
  });
  zoomAnim = new Animated.Value(1);
  pinAnim = new Animated.ValueXY({
    x: 0,
    y: 0
  });
  _ignorePagingNext = false;
  _lockAxisForGesture = null;
  __offsets = {
    x: {
      value: 0,
  ...
  __setOffset(axis, offset) {
    ...
    if (this.props.bindToBorders) {
      ...
      const boundOffset = contentSize && containerSize && this.props.panBoundaryPadding != null ? applyPanBoundariesToOffset(offset, containerSize, contentSize, this.zoomLevel, this.props.panBoundaryPadding) : offset;
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const staticPinPositionChanged = prevProps.staticPinPosition?.x !== this.props.staticPinPosition?.x || prevProps.staticPinPosition?.y !== this.props.staticPinPosition?.y;
  ...
  debouncedOnStaticPinPositionChange = debounce(position => this.props.onStaticPinPositionChange?.(position), 100);
  ...
  measureZoomSubject = () => {
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        ...
        this.zoomSubjectWrapperRef.current?.measure((x, y, width, height, pageX, pageY) => {
  ...
  _handlePanResponderEnd = (e, gestureState) => {
    ...
    const disableMomentum = this.props.disableMomentum || this.props.panEnabled && this.gestureType === 'shift' && this.props.disablePanOnInitialZoom && this.zoomLevel === this.props.initialZoom;
  ...
  _resolveAndHandleTap = e => {
    ...
    if (this.doubleTapFirstTapReleaseTimestamp && this.props.doubleTapDelay && now - this.doubleTapFirstTapReleaseTimestamp < this.props.doubleTapDelay) {
      this.doubleTapFirstTap && this._addTouch({
  ...
  _handleDoubleTap(e) {
    ...
    onDoubleTapAfter?.(e, this._getZoomableViewEventObject({
  ...
  zoomTo(newZoomLevel, zoomCenter) {
    ...
    if (zoomCenter) {
      ...
      listenerId = this.zoomAnim.addListener(({
        ...
        this.panAnim.setValue({
          x: calcNewScaledOffsetForZoomCentering(this.offsetX, this.state.originalWidth, prevScale, newScale, zoomCenter.x),
          y: calcNewScaledOffsetForZoomCentering(this.offsetY, this.state.originalHeight, prevScale, newScale, zoomCenter.y)
        });
        prevScale = newScale;
      });
    }
  ...
  render() {
    ...
    return /*#__PURE__*/_jsxs(View, {
      ...
      children: [/*#__PURE__*/_jsx(Animated.View, {
        ...
        children: children
      }), visualTouchFeedbackEnabled && touches?.map(touch => doubleTapDelay && /*#__PURE__*/_jsx(AnimatedTouchFeedback, {
        x: touch.x,
        y: touch.y,
